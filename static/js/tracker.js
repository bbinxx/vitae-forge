...
const MASTER_INSTRUCTION = {
    "master_instruction": {
        ...
        "resume_variant_selection": {
            ...
            "default_resume_variant": "RESUME_SD.pdf"
        },
        ...
        "variant_resolution_rule": "Choose the most specialized variant. If multiple qualify equally, use RESUME_SD.pdf.",
            ...
            <div id="email-edit-mode" style="display:flex;flex-direction:column;gap:10px;flex:1">
                ...
                <div class="field-group">
                    ...
                    <input id="email-subject" type="text" class="input-field"
                        placeholder="e.g. Application for Java Intern –  Candidate"
                        oninput="window.onEmailFieldChange()"
                        style="font-size:12px" />
                </div>
        ...
        <div style="padding:16px 24px;height:calc(90vh - 160px);min-height:460px;display:flex;flex-direction:column;gap:12px;overflow-y:auto">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                ...
                <div class="field-group">
                    ...
                    <input id="dt-assigned-pdf" class="input-field" placeholder="RESUME_SD.pdf" oninput="window.onDetailsFieldChange()" style="font-size:12px" />
                </div>
            </div>
...
function buildFullEmailBody(body) {
    ...
    const name = sig.name || ' Candidate';
    const phone = sig.phone || '[Your Phone]';
    const email = sig.email || '[your@email.com]';
...
window.setEmailMode = (mode) => {
    ...
    if (mode === 'preview') {
        ...
        if (box) {
            const sig = (MASTER_INSTRUCTION.master_instruction?.email_rules?.signature) || {};
            const senderName  = sig.name || ' Candidate';
            const senderEmail = sig.email || '[your@email.com]';
