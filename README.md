...
./scripts/watch_preview.sh roles/standard/Bibin_Raju_Resume.tex
```

### 2. PDF & Release Automation
*   **Automatic Compilation**: Every commit to the `main` branch triggers a GitHub Action that compiles your default resume (`roles/standard/Bibin_Raju_Resume.tex`) and updates the preview image.
...
```bash
RESUME_TEX=roles/standard/Bibin_Raju_Resume.tex python3 scripts/to_json.py
```
