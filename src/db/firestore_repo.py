...
class FirestoreRepository(AbstractRepository):
    ...
    def get_templates(self, user_id: str) -> List[Dict[str, Any]]:
        docs = self._user_ref(user_id).collection("templates").stream()
        result = []
        for doc in docs:
            data = doc.to_dict()
            if data and "id" not in data:
    ...
    def get_template(self, user_id: str, template_id: str) -> Optional[Dict[str, Any]]:
        doc = self._user_ref(user_id).collection("templates").document(template_id).get()
        if not doc.exists:
            return None
        data = doc.to_dict()
        if "id" not in data:
    ...
    def save_template(self, user_id: str, template: Dict[str, Any]) -> None:
        template_id = template.get("id")
        if not template_id:
            return
        self._user_ref(user_id).collection("templates").document(template_id).set(template)

    def delete_template(self, user_id: str, template_id: str) -> None:
        self._user_ref(user_id).collection("templates").document(template_id).delete()
